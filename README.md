...
| 항목          | 현재 상태                                                                                      |
| ------------- | ---------------------------------------------------------------------------------------------- |
| 프로젝트 단계 | M1 플랫폼 골격·M2 작가 스튜디오 완료, M4 로컬 이미지 생성 기술 기반 선행 완료                  |
| 구현된 기반   | 인증, 작품·회차·자동 저장·수정 이력, 캐릭터·세계관·사건 DAG, 이미지 자산과 Windows Worker 검증 |
| 다음 목표     | 고정된 회차 버전을 사용하는 발행 모델과 반응형 독자 뷰어                                       |
...
flowchart LR
    ...
    LOCAL["Windows Local Image Worker"] -. "M4 작업 조회 계획" .-> API
...
| 구현 완료 | 캐릭터 이미지 자산·참조 팩과 Windows Worker의 ComfyUI 사전 검사·고정 워크플로 실행·로컬 결과 회수 |
| 설계 완료 | 운영 DB 분리, Local-first 이미지 실행과 비GPU 사용자를 위한 서버·외부 API 대체 경계               |
| 다음 구현 | 고정 `ChapterRevision` 발행 모델, 작가 발행·취소 흐름과 반응형 독자 뷰어                          |
| 후속 계획 | 근거 기반 AI 일관성 검사, Worker 작업 계약·선택 업로드와 대체 이미지 제공자                       |
...
| M4 로컬 이미지 생성   | 대기 | 이미지 자산·로컬 실행 기반 완료, 작업 계약·선택 업로드 대기 |
| M5 발행과 독자 검증   | 준비 | 작품 생성부터 회차 발행·열람까지 검증                       |
