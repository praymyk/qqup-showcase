...
flowchart LR
    ...
    DOCS --> COMMIT["주제별 커밋 · 원격 브랜치 푸시"]
    COMMIT --> REPORT["결과 · 근거 · 남은 위험 보고"]
```
...
변경 작업은 최신 `main`에서 분기한 `codex/<주제>` 브랜치에 격리한다. 검증 가능한 작업 단위마다 관련 파일만 커밋하고, 검사를 통과하면 원격 주제 브랜치까지 푸시한다. 이를 통해 다른 환경에서도 같은 변경을 곧바로 검토하거나 이어서 작업할 수 있다.

반면 `main` 병합은 자동화 범위에서 제외한다. 작업 브랜치의 검사 결과와 변경 범위를 사람이 확인한 뒤 명시적으로 승인해야 병합한다. 이 경계는 반복적인 전달 과정은 자동화하면서도 기준 브랜치의 변경 결정과 공개 시점을 사람이 통제하기 위한 것이다. 강제 푸시, 자동 병합과 배포도 별도 승인 없이는 수행하지 않는다.
...
이 공통 규칙 위에서 기능 구현의 반복 절차만 프로젝트 Skill로 묶었다. 여러 에이전트와 외부 자동화를 항상 실행하지 않고 작업 크기에 따라 검증 단계를 선택하는 이유와 호출 방법은 [개인 개발자를 위한 최소 AI 개발 루프](minimum-viable-ai-loop.md)에서 설명한다.
