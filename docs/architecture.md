...
flowchart LR
    ...
    WORKER["Windows Local Image Worker"] -. "M4 외부 방향 작업 조회" .-> API
...
| Windows Local Image Worker | 사용자 장치에서 작업 조회·사전 검사·이미지 생성·미리보기 | 인바운드 서버 역할, 공개 승인과 작품 권한 판단   |
...
M1 플랫폼 골격과 M2 작가 스튜디오는 완료했다. 작품·회차·자동 저장·수정 이력, 캐릭터·세계관, 사건 시간축 DAG와 회차 배치까지 비공개 집필 흐름을 구현했다. 현재 다음 목표는 고정된 회차 버전을 발행하고 독자 화면에서 읽는 M5다.

M4에서는 캐릭터 이미지 자산과 참조 팩, Windows Worker의 ComfyUI 사전 검사·고정 워크플로 실행·로컬 결과 회수까지 기술 기반을 검증했다. Worker–Core API 작업 계약, 로컬 미리보기·선택 업로드와 대체 제공자는 후속 계획이다. AI Service 골격도 실제 모순 검사 모델 연동 완료를 의미하지 않는다.
